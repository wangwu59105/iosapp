...
@objc protocol PageContentViewDelegate : NSObjectProtocol {
    func pageContentView(_ contentView: PageContentView, progress: CGFloat, sourceIndex: Int, targetIndex: Int)
    @objc optional func pageContentView(_ contentView: PageContentView, currentPageIndex: NSNumber)
}
...
extension PageContentView: UICollectionViewDelegate {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        let sel = #selector(PageContentViewDelegate.pageContentView(_:currentPageIndex:))
        if (delegate?.responds(to: sel) == true) {
            _ = delegate?.perform(sel, with: self, with: NSNumber.init(value: index))
            //delegate?.pageContentView(self, currentPageIndex: index)
        }
    }
